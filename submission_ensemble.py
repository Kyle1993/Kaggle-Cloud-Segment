...
save_dir = os.path.join(base_folder,'{}_{}_{}'.format(ensmeble_fold,cls_fold,time_str))
...
with open(os.path.join(save_dir,'config'), 'w') as f:
    f.write('{}\n\n{}'.format(time_str, utils.config2str({'ensmeble_fold':ensmeble_fold, 'cls_fold':cls_fold})))

# search thresholds
seg_probs = np.load(os.path.join(SAVE_PATH,'segment',ensmeble_fold,'seg_probs.npy'))
...
cloud_mask = np.load(os.path.join(SAVE_PATH,'segment',ensmeble_fold,'cloud_mask.npy'))
