...
config = {'MODEL':'UNET',
          ...
          'LR':5e-4,
          'WD':1e-5,
          'note':'wd-1e-5',# ReduceLROnPlateau-0.5-20, MultiStepLR-5,20-0.025, MultiStepLR-10,14,16,18,20,22,24-0.2
          'train_time':time_str,
          }
...
if MODEL == 'UNET':
    ...
    model = smp.FPN(encoder_name=ENCODER, classes=4, activation='sigmoid')
else:
    model = None
...
for epoch in range(NUM_EPOCH):
    ...
    for i,(img_ids, imgs, masks, _) in enumerate(dataloader):
        ...
        if global_step % VALIDATE_STEP == 0:
            ...
            model.eval()
            with torch.no_grad():
                _, imgs, masks, _ = validate_dataset.sample(BATCH_SIZE * 3)
                imgs = imgs.float().cuda(GPU)
                masks = masks.float().cuda(GPU)
                outputs = model(imgs)
                loss = criterion(outputs, masks)
